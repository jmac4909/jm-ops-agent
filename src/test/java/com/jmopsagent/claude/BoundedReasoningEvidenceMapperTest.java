...
class BoundedReasoningEvidenceMapperTest {
    ...
    void freshlyCollectedOldIncidentEvidenceStillReachesReasoningAfterPriorEvidenceFillsThePrompt() {
        ...
        for (int index = 0; index < 30; index++) items.add(item("Old collection " + index, "x".repeat(25_000)));
        var fresh = item("New historical source finding", "Missing projected field in old revision");
        items.add(fresh);
        var superseded = item("Previous scope", "Superseded source");
    ...
    private EvidenceItem item(String summary, String content) {
        return EvidenceItem.create(EvidenceSource.GITLAB, EvidenceType.SOURCE_CODE, Instant.parse("2024-05-01T00:00:00Z"),
