...
The service registry's `tas.appPattern` takes precedence over the global pattern. Set `runtime.platform.<ENV>` to `TAS` when a service has both TAS and EKS metadata; this keeps runtime selection explicit and makes approved follow-on requests use the same adapter. Recent CF logs and `cf events` are output-bounded and depend on CLI retention. Initial TAS triage collects both; events retain deployment actors, times and operation types.
...
Dependency investigation follows bounded registered relationships and uniquely resolved Feign destinations. Each dependency receives its own runtime, deployment and GitLab evidence collection. Explicit registered GET fixtures can compare selected good/bad response fields. Data-store adapters, including DynamoDB, remain future work. See [Dependency triage setup](dependency-triage.md) for opt-in probes, config-server lookup, service targeting and evidence limitations.
